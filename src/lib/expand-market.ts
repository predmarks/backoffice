...
type MarketField = (typeof EXPANDABLE_FIELDS)[number];

function getMissingFields(partial: Record<string, unknown>): MarketField[] {
  return EXPANDABLE_FIELDS.filter((f) => {
    const val = partial[f];
...
/**
 * Uses Claude to fill missing fields on a partial market object.
 * Returns only the generated fields (caller merges).
 */
export async function expandMarket(
